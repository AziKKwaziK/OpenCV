...
detector = cv2.CascadeClassifier(f"{os.getcwd()}/haarcascade_frontalface_default.xml")
...
def getImagesAndLabels(path):       #method to get the images and label data
    ...
    # O(nm) n - number of pictures, m - number of faces per picture 
...
faces,ids = getImagesAndLabels(f'{os.getcwd()}/dataset')  # Get the faces and IDs
recognizer.train(faces, np.array(ids)) # Train the model using the faces and IDs
print(time.time() - start_time)
assure_path_exists(f'{os.getcwd()}/trainer/')    # Save the model into trainer.yml
recognizer.save(f'{os.getcwd()}/trainer/trainer.yml')
